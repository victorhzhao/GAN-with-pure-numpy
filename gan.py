...
class GAN(object):
	def __init__(self, numbers):
		...
		#if you don't get expecting result when the program had stoped make the ecpoch larger
		self.epochs = 30
		self.batch_size = 64
	...
	def train(self):
		...
		#init plt
		#imshowed = plt.imshow(np.zeros((256, 256), dtype="float32"), cmap="gray")
		plt.ion()
		#set batch indices
		...
		for epoch in range(self.epochs):
			for idx in range(batch_idx):
				...
				#if you don't want to see the result at every 30 steps, comment 2 lines below
				if idx%30 == 0: #for speed consideration only display update every 30 steps
...
numbers = n
print("generate num", str(numbers))
gan = GAN(numbers)
gan.train()
print("30 epach completed")
plt.ioff()
plt.show()